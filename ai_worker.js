...
function handleRequestMove(payload) {
    ...
    // ▼▼▼ この行を修正 ▼▼▼
    // 元のコード: Module.updateGameState(board, currentMino.charCodeAt(0), holdMino.charCodeAt(0), nextMinos, b2b, ren);
    // 修正後:
    const uint16Board = boardToUint16Array(board); // 盤面データをUint16Arrayに変換
    Module.updateGameState(uint16Board, currentMino.charCodeAt(0), holdMino.charCodeAt(0), nextMinos, b2b, ren);
    // ▲▲▲ 修正ここまで ▲▲▲
